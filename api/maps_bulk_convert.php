...
function resolve_maps_coords_bulk(string $url): array
{
    $direct = extract_maps_coords_bulk($url);
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_USERAGENT => 'Mozilla/5.0 (compatible; BillingMapsBulkResolver/1.0)',
    ]);
    curl_exec($ch);
    ...
    $coords = extract_maps_coords_bulk($resolved);
...
$sql = "SELECT id, username, maps FROM users WHERE {$where} ORDER BY id DESC LIMIT {$limit}";
...
$update = $conn->prepare("UPDATE users SET lat = ?, lng = ?, maps = ?, updated_at = NOW() WHERE id = ?");
foreach ($rows as $row) {
    $processed++;
    $result = resolve_maps_coords_bulk($row['maps']);
    if (!empty($result['success'])) {
        ...
        $update->execute();
        $updated++;
        $items[] = ['id' => $id, 'username' => $row['username'], 'success' => true, 'lat' => $lat, 'lng' => $lng, 'source' => $result['source'] ?? 'unknown'];
    } else {
        $failed++;
        $items[] = ['id' => (int)$row['id'], 'username' => $row['username'], 'success' => false, 'message' => $result['message'] ?? 'Gagal'];
    }
}
$update->close();

$countSql = "SELECT COUNT(*) AS remaining FROM users WHERE {$where}";
